    ...
    <>
      ...
      {!isLoggedIn ? (
        <div className={styles.mustBeLoggedInMsg}>
          <p>
            You must be <Link to={"/login"}>logged in</Link> as an author to have a profile
          </p>
        </div>
      ) : (
                  ...
                  {errorMsgs == null ? null : (
                    <ul className={styles.errorMsgsUl}>
                      {errorMsgs.map((err, indx) => {
                        return (
                          <li className={styles.errorMsgsLi} key={indx}>
                            {err.msg}
                          </li>
                        );
                      })}
                  ...
                  <div className={styles.labelAndTextAreaCont}>
                    ...
                    {isAuthor == null ? null : (
                      <div className={styles.countAndTextAreaCont}>
                        <p className={styles.bioInputCount}>
                          {isAuthor.bio.length} / 255
                        </p>
                        <textarea
                          maxLength={255}
